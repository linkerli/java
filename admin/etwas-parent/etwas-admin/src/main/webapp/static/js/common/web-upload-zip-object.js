...
(function() {
    ...
    var $WebZipUpload = function(pictureId) {
    ...
    $WebZipUpload.prototype = {
        ...
        bindEvent : function(bindedObj) {
            ...
            bindedObj.on('uploadProgress', function(file, percentage) {
                ...
                $("#"+me.uploadBarId).css("width",percentage * 100 + "%").text((percentage * 100).toFixed(2) + '%');
            });

            // 文件上传成功，给item添加成功class, 用样式标记上传成功。
            bindedObj.on('uploadSuccess', function(file,response) {
                Feng.success("上传成功");
                $("#" + me.uploadPreId).html('<a href="'+ response +'" target="_blank">'+ response +'</a>');
                $("#"+me.uploadBarId).css("width", '0%');
    ...
    window.$WebZipUpload = $WebZipUpload;
