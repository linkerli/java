<template>
  ...
  >
    ...
    <img
      class="img"
      :src="selectedImg"
      :style="{ width: width +'px', height: height + 'px', opacity: issuccess == true?1.0:0.0 }"
    />
    ...
    <div
      ...
      :index="item"
      :key="item"
      :class="['piece', 'piece-'+(item) ,item == boxArractivelass ? 'active' : '', issuccess == true?'hide':'' ]"
      :style="{width: width/3 +'px' }"
      @click="changePositon($event,item)"
      :ref="'piece' +item"
    >
      <img class="item-inner" />
    </div>
  </div>
</template>
